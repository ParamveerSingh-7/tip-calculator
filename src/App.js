...
function Calculator() {
  ...
  const [persons, setPersons] = useState(0.0);
  const amountPerPerson = persons > 0 ? amount / persons : null;
  const tipPerPerson =
    persons > 0 ? ((amount * (tip / 100)) / persons).toFixed(2) : null;
  const totalPerPerson = (amountPerPerson + Number(tipPerPerson)).toFixed(2);
  ...
  return (
    <div className="calculator">
      <div className="left">
        ...
        <div className="tipPercentageSection">
          <p>Select Tip %</p>
          <div className="btnsHolder">
            <button
              id="5"
              className={`btn ${tip === 5 ? "active" : ""}`}
              onClick={() => {
                setTip(5);
              }}
            >
            ...
            <button
              className={`btn ${tip === 10 ? "active" : ""}`}
              onClick={() => setTip(10)}
            >
              10%
            </button>
            <button
              className={`btn ${tip === 15 ? "active" : ""}`}
              onClick={() => setTip(15)}
            >
              15%
            </button>
            <button
              className={`btn ${tip === 25 ? "active" : ""}`}
              onClick={() => setTip(25)}
            >
              25%
            </button>
            <button
              className={`btn ${tip === 50 ? "active" : ""}`}
              onClick={() => setTip(50)}
            >
              50%
            </button>
        ...
        <div className="peopleSection">
          <div class="error">
            ...
            <span
              className={`er ${persons === 0 && tip > 0 ? "" : "invisible"}`}
            >
              Can't be zero
            </span>
          </div>
          <label class="input-label">
            ...
            <input
              type="text"
              id="input-box"
              class={`input-field ${persons === 0 && tip > 0 ? "er" : ""}`}
              placeholder="0"
      ...
      <div className="right">
        <div className="amountsHolder">
          <div className="tipPerPerson">
            <div className="az">
              <p>Tip Amount</p>
              <span>/ person</span>
            </div>
            <h2 className="amount">${tipPerPerson > 0 ? tipPerPerson : 0}</h2>
          </div>
          <div className="totalPerPerson">
            ...
            <h2 className="amount">
              ${totalPerPerson > 0 ? totalPerPerson : 0}
            </h2>
          </div>
        </div>
        <button
          className={`${amount > 0 ? "" : "defuse"} resetBtn`}
          onClick={handleReset}
        >
          RESET
        </button>
